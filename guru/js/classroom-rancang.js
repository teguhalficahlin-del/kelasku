...
<div class="rp-nav-row" style="justify-content:space-between;">
  ...
  <button type="button" class="rp-btn-next" id="rp-step1-ro-next">
    ...
    async function autoGenerateDanSimpanCp() {
      ...
      // Wali SD: sudahAdaCp = semua mapel di mapel_list sudah punya dokumen CP sendiri.
      // Non-Wali SD: cukup ada satu dokumen CP.
      let sudahAdaCp;
      let mapelListToGenerate = [];

      if (isWaliSd) {
        const _mapelArr = normalizeArray(_profil?.mapel_list).filter(Boolean);
        const fullMapelList = _mapelArr.length ? _mapelArr : [_ans.mapel].filter(Boolean);
        mapelListToGenerate = fullMapelList.filter(m =>
          !_dokumen.some(d => d.jenis === 'CP' && d.judul === `CP — ${m}`)
        );
        sudahAdaCp = fullMapelList.length > 0 && mapelListToGenerate.length === 0;
      } else {
        sudahAdaCp = _dokumen.some(d => d.jenis === 'CP');
      }
      ...
      // CP belum tersimpan (sebagian atau seluruhnya) — generate + simpan otomatis
      if (isWaliSd) {
        if (!mapelListToGenerate.length) { renderNavRowCp(); return; }
        if (navWrap) navWrap.innerHTML = `<div class="rp-nav-row"><span class="rp-identitas-status">Menyiapkan ringkasan CP…</span></div>`;
        let anyFailed = false;
        for (let i = 0; i < mapelListToGenerate.length; i++) {
          const m = mapelListToGenerate[i];
          const mk = normalizeMapelKey(m);
          if (navWrap) navWrap.innerHTML = `<div class="rp-nav-row"><span class="rp-identitas-status">Menyiapkan CP ${esc(m)}… (${i + 1}/${mapelListToGenerate.length})</span></div>`;
          try {
