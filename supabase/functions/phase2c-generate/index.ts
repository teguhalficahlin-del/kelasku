...
const LOCKED_ROLES = new Set(['WALI_KELAS_SD','GURU_MAPEL_SDSMP_SMA','GURU_MAPEL_UMUM_SMK','GURU_MAPEL_PRODUKTIF_SMK']);
// Gate Tab Rancang -- dua syarat dan keduanya wajib: peran mengajar
// GURU_MAPEL_UMUM_SMK dan tier GURU_PRO. LOCKED_ROLES di atas sengaja
// dibiarkan utuh; set itu disiapkan untuk Rancang V2, dan gate ini syarat
// tambahan di atasnya, bukan penggantinya.
  ...
  try {
    ...
    if (action === 'generate_context_spec' || action === 'regenerate_context_spec') {
      ...
      const { data: validationRaw } = await admin
        .rpc('fn_phase2c_validate_context_spec', { p_content: asJson(content) });
      ...
      if (!createResult.idempotent) {
        ...
        await admin.rpc('fn_phase2b_transition_version', {
          ...
          p_validation_summary: validation,
          p_reason: sqlNull(null), p_idempotency_key: await makeIdempotencyKey('ctx_validate', versionId),
        });
      }
    ...
    if (action === 'save_context_spec_edit') {
      ...
      const { data: validationRaw } = await admin
        .rpc('fn_phase2c_validate_context_spec', { p_content: asJson(editedContent) });
      const validation = validationRaw as ValidationResult | null;
      if (validation?.status !== 'valid')
    ...
    if (action === 'confirm_context_spec') {
      ...
      const { data, error } = await admin.rpc('fn_phase2b_transition_version', {
        p_profile_id: profile.id, p_version_id: versionId,
        p_action: 'CONFIRM', p_validation_status: sqlNull(null), p_validation_summary: null,
    ...
    if (action === 'generate_assessment_spec' || action === 'regenerate_assessment_spec') {
      ...
      const { data: validationRaw } = await admin
        .rpc('fn_phase2c_validate_assessment_spec', { p_content: asJson(content) });
      const validation = validationRaw as ValidationResult | null;
      if (validation?.status !== 'valid') {
    ...
    if (action === 'save_assessment_spec_edit') {
      ...
      const { data: validationRaw } = await admin
        .rpc('fn_phase2c_validate_assessment_spec', { p_content: asJson(editedContent) });
      const validation = validationRaw as ValidationResult | null;
      if (validation?.status !== 'valid')
        return reply({ error: 'Edit tidak memenuhi schema', violations: validation?.violations }, 400);
