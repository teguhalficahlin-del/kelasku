-- 20260823000006_fn-tahun-ajaran-reset.sql
-- Reset tahun ajaran -- hak eksklusif tier GURU_PRO, sekali setahun.
--
-- Bedanya dengan fn_semester_reset bukan pada apa yang dihapus, melainkan pada
-- siapa yang boleh dan seberapa sering. Data operasional yang disapu persis
-- sama; yang membuat GURU_PRO berbeda adalah Tab Rancang, yang justru
-- DIPERTAHANKAN lintas tahun -- perangkat ajar yang disusun tahun ini dipakai
-- lagi tahun depan, itulah nilai yang dibeli.
--
-- Daftar tabel disalin dari fn_semester_reset versi 20260821000011. Tiga tabel
-- rubrik warisan (assessment_rubric_results, assessment_kktp_results,
-- assessment_rubric_criteria) sengaja tidak ada: ketiganya sudah di-DROP di
-- migrasi itu, menyebutnya di sini hanya akan membuat fungsi gagal dibuat.
--
-- Urutan penghapusan mengikuti arah FK -- anak dulu, induk belakangan.

CREATE OR REPLACE FUNCTION public.fn_tahun_ajaran_reset(p_teacher_id uuid)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO 'public'
AS $function$
DECLARE
  v_classrooms uuid[];
  v_hasil      jsonb := '{}'::jsonb;
  v_n          bigint;
  v_tier       text;
BEGIN
  IF p_teacher_id IS NULL THEN
    RAISE EXCEPTION 'p_teacher_id wajib diisi';
  END IF;

  IF NOT EXISTS (
    SELECT 1 FROM profiles
    WHERE id = p_teacher_id AND role = 'GURU'
  ) THEN
    RAISE EXCEPTION 'Profil % bukan GURU atau tidak ditemukan', p_teacher_id;
  END IF;

  SELECT tier INTO v_tier FROM profiles WHERE id = p_teacher_id;

  IF v_tier IS DISTINCT FROM 'GURU_PRO' THEN
    RAISE EXCEPTION 'Reset tahun ajaran hanya tersedia untuk GURU_PRO';
  END IF;

  SELECT coalesce(array_agg(id), ARRAY[]::uuid[])
    INTO v_classrooms
    FROM classrooms
   WHERE teacher_id = p_teacher_id;

  IF array_length(v_classrooms, 1) IS NOT NULL THEN

    DELETE FROM attendance                WHERE classroom_id = ANY(v_classrooms);
    GET DIAGNOSTICS v_n = ROW_COUNT; v_hasil := v_hasil || jsonb_build_object('attendance', v_n);

    DELETE FROM assessment_results        WHERE classroom_id = ANY(v_classrooms);
    GET DIAGNOSTICS v_n = ROW_COUNT; v_hasil := v_hasil || jsonb_build_object('assessment_results', v_n);

    DELETE FROM grade_recap               WHERE classroom_id = ANY(v_classrooms);
    GET DIAGNOSTICS v_n = ROW_COUNT; v_hasil := v_hasil || jsonb_build_object('grade_recap', v_n);

    DELETE FROM assessments               WHERE classroom_id = ANY(v_classrooms);
    GET DIAGNOSTICS v_n = ROW_COUNT; v_hasil := v_hasil || jsonb_build_object('assessments', v_n);

    DELETE FROM tp_kktp                   WHERE classroom_id = ANY(v_classrooms);
    GET DIAGNOSTICS v_n = ROW_COUNT; v_hasil := v_hasil || jsonb_build_object('tp_kktp', v_n);

    DELETE FROM schedules                 WHERE classroom_id = ANY(v_classrooms);
    GET DIAGNOSTICS v_n = ROW_COUNT; v_hasil := v_hasil || jsonb_build_object('schedules', v_n);

    DELETE FROM student_notes             WHERE classroom_id = ANY(v_classrooms);
    GET DIAGNOSTICS v_n = ROW_COUNT; v_hasil := v_hasil || jsonb_build_object('student_notes', v_n);

    DELETE FROM guidance_sessions         WHERE classroom_id = ANY(v_classrooms);
    GET DIAGNOSTICS v_n = ROW_COUNT; v_hasil := v_hasil || jsonb_build_object('guidance_sessions', v_n);

    DELETE FROM student_groups            WHERE classroom_id = ANY(v_classrooms);
    GET DIAGNOSTICS v_n = ROW_COUNT; v_hasil := v_hasil || jsonb_build_object('student_groups', v_n);

    DELETE FROM parent_messages           WHERE classroom_id = ANY(v_classrooms);
    GET DIAGNOSTICS v_n = ROW_COUNT; v_hasil := v_hasil || jsonb_build_object('parent_messages', v_n);

    -- forum_comments ikut lewat FK post_id ON DELETE CASCADE.
    DELETE FROM forum_posts               WHERE classroom_id = ANY(v_classrooms);
    GET DIAGNOSTICS v_n = ROW_COUNT; v_hasil := v_hasil || jsonb_build_object('forum_posts', v_n);

    -- TIDAK dihapus, disengaja: classrooms, classroom_roster,
    -- classroom_members, dan profiles siswa/ortu -- guru memulai tahun ajaran
    -- baru dengan kelas dan daftar siswa yang sama.
    --
    -- PLACEHOLDER: tabel Tab Rancang juga TIDAK dihapus -- perangkat ajar
    -- dipertahankan lintas tahun. Schema Tab Rancang belum final; setelah
    -- selesai, tulis daftar tabelnya di sini sebagai komentar eksplisit
    -- (bukan DELETE) supaya keputusan ini terbaca oleh migrasi berikutnya.

  END IF;

  UPDATE profiles
     SET last_reset_at = now()
   WHERE id = p_teacher_id;

  RETURN jsonb_build_object(
    'success',    true,
    'classrooms', coalesce(array_length(v_classrooms, 1), 0),
    'dihapus',    v_hasil
  );
END;
$function$;

-- Sama seperti fn_semester_reset: hanya service_role (Edge Function) yang boleh
-- memanggil. Fungsi ini SECURITY DEFINER dan menerima teacher_id sebagai
-- argumen -- akses dari browser berarti guru mana pun bisa menghapus data guru
-- lain hanya dengan mengganti satu uuid.
REVOKE ALL ON FUNCTION fn_tahun_ajaran_reset(uuid) FROM PUBLIC;
REVOKE ALL ON FUNCTION fn_tahun_ajaran_reset(uuid) FROM anon;
REVOKE ALL ON FUNCTION fn_tahun_ajaran_reset(uuid) FROM authenticated;
GRANT EXECUTE ON FUNCTION fn_tahun_ajaran_reset(uuid) TO service_role;

-- TODO: Edge Function pemanggil fn_tahun_ajaran_reset belum dibuat.
-- Akan dikerjakan setelah schema Tab Rancang selesai dan tabel Tab Rancang
-- ditambahkan ke daftar DELETE di fungsi ini.
-- Pola yang harus diikuti: supabase/functions/semester-reset/index.ts
